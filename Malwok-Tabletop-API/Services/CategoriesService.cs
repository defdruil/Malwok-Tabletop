...
namespace Malwok_Tabletop_API.Services
{
    public static class CategoriesService
    {
        ...
        public static bool InitCategories()
        {
            try
            {
                List<string> categoriesPaths = new List<string>(Directory.EnumerateDirectories(AppDomain.CurrentDomain.BaseDirectory + "..\\Malwok-Tabletop\\Resources"));
                using (var context = new MalwokAudioContext())
                {
                    // Nettoyage de la BDD
                    /*context.SaveChanges();
*/
                    foreach (string categoryPath in categoriesPaths)
                    {
                        ...
                        foreach (string playlistPath in playlistsPaths)
                        {
                            ...
                            foreach (string soundpath in soundsPaths)
                            {
                                ...
                                Sound sound = new Sound() { Name = soundName, PlaylistId = playlistId, Path = getRelativePath(soundpath) };
        ...
        {
            return path.Substring(AppDomain.CurrentDomain.BaseDirectory.Length + "Malwok-Tabletop/".Length + 2);
        }
